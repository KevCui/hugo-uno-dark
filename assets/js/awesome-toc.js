(function ($) {
  ...
  var generateTableOfContents = function (documentRef) {
    ...
    var contentRef = documentRef.body
    ...
    if (headingList.length > 0) {
      var headings = [].slice.call(
        contentRef.querySelectorAll(headingList.join(', '))
      )
      headings.forEach(function (heading, index) {
        ...
        $(link).click(function (e) {
          $(this)
            ...
            .parent()
            .find('a')
            .css({ color: baseConfig.css.fontColor })
          $(this).css({ color: baseConfig.css.activeFontColor })
          var offsetTop =
            $($(this).attr('href')).offset().top - baseConfig.topOffset
  ...
  var getIndent = function (currentHeading, headingList, baseIndent) {
    ...
    for (; position < headingList.length; position++) {
      if (currentHeading !== headingList[position]) continue
      else break
    }
  ...
  var toggleButtonClickListener = function () {
    ...
    if (sideBar.hasClass(baseConfig.sideBarPrefix + 'active')) {
      // 隐藏
  ...
  var scrollHandler = function () {
    ...
    if (scrollVar.lastId !== id && !id.includes('sidebar-toc-Ik4D')) {
      scrollVar.lastId = id
      scrollVar.menuItems.css({ color: baseConfig.css.fontColor })
      scrollVar.menuItems
        .filter('[href=#' + id + ']')
        .css({ color: baseConfig.css.activeFontColor })
    }
  }
  ...
  var getNiceHeadingTags = function (firstThreshold, level) {
    ...
    for (var idx = 0; idx < allHeadingTags.length; idx++) {
      var heading = allHeadingTags[idx]
      if (result.length === 0) {
  ...
  var isToggleBtnExist = function () {
    if ($('#' + baseConfig.sideBarPrefix + 'toggleBtn').length === 0) { return false } else return true
  }

  var isToTopBtnExist = function () {
    if ($('#' + baseConfig.sideBarPrefix + 'toTopBtn').length === 0) { return false } else return true
  }
